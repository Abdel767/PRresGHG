...
site_lake_combos <- unique(paste(gga_3$site_id, gga_3$lake_id))
n <- length(site_lake_combos)
...
# Assuming data.gga.ch4.list and data.gga.co2.list are defined and contain data for multiple sites
for (i in 1:length(data.gga.ch4.list)) {
  ...
  c.initial.ch4 <- data.gga.ch4.list[[i]]$CH4._ppm[min(which(!is.na(data.gga.ch4.list[[i]]$CH4._ppm)))]  # initial CH4
  exp.ch4.i <- try(nlsLM(CH4._ppm ~ cmax - (cmax - b) * exp(-k * as.numeric(elapTime)),
                         data = data.gga.ch4.list[[i]], start = list(cmax = cmax.ch4, b = cmax.ch4 - c.initial.ch4, k = 0.03)),
                   silent = TRUE)
  ...
  # Converting from parts per million to umole cross out. No conversion factor necessary. Dome area = 0.2 m2
  ch4.lm.drate.i.umol.s <- ((volume.i * 1 * slope.ch4.i) / (0.082057 * (temp.i + 273.15))) / 0.2  # umol CH4 s-1
  ch4.lm.drate.mg.h <- if (is.na(ch4.lm.drate.i.umol.s)) NA else ch4.lm.drate.i.umol.s * (16/1000) * (60*60)  # mg CH4 m-2 h-1
  
  co2.lm.drate.i.umol.s <- ((volume.i * 1 * slope.co2.i) / (0.082057 * (temp.i + 273.15))) / 0.2  # umol CO2 s-1
  co2.lm.drate.mg.h <- if (is.na(co2.lm.drate.i.umol.s)) NA else co2.lm.drate.i.umol.s * (44/1000) * (60*60)  # mg CO2 m-2 h-1
  
  ch4.ex.drate.i.umol.s <- ((volume.i * 1 * ch4.ex.slope) / (0.082057 * (temp.i + 273.15))) / 0.2  # umol CH4 s-1
  ch4.ex.drate.mg.h <- if (is.na(ch4.ex.drate.i.umol.s)) NA else ch4.ex.drate.i.umol.s * (16/1000) * (60*60)  # mg CH4 m-2 h-1
  
  co2.ex.drate.i.umol.s <- ((volume.i * 1 * co2.ex.slope) / (0.082057 * (temp.i + 273.15))) / 0.2  # umol CO2 s-1
  co2.ex.drate.mg.h <- if (is.na(co2.ex.drate.i.umol.s)) NA else co2.ex.drate.i.umol.s * (44/1000) * (60*60)  # mg CO2 m-2 h-1
  ...
  results[[i]] <- result  # Add the result to the list
}

# Combine all results into a single data frame
OUTb <- do.call(rbind, results)
...
bysch4 <- OUT2 %>%
  ...
  mutate(vID=paste(lake_id))%>%
  group_by(vID)%>%
  summarise(lake_id[1],ch4_drate_mg_h=mean(ch4_drate_mg_h_best),
...
bysco2 <- OUT2 %>%
  ...
  mutate(vID=paste(lake_id))%>%
  group_by(vID)%>%
  summarise(lake_id[1],co2_drate_mg_h=mean(co2_drate_mg_h_best))
