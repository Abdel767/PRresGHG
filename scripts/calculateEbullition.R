...
gc_lakeid_agg <- read.csv("gc_lakeid_agg.csv")

##############################################################################################################################

# Make sure lake_id and site_id exist and match types
gc_lakeid_agg <- gc_lakeid_agg %>%
  mutate(
    site_id = gsub("S-", "", SiteID),  # extract numeric part from SiteID
    site_id = as.character(site_id),
    lake_id = "1000"  # if only one lake ID, else adjust accordingly
  )

gc_lakeid_agg <- gc_lakeid_agg %>%
  mutate(
    lake_id = stringr::word(sample, 1, sep = "_"),
    site_id = stringr::word(sample, 2, sep = "_")
  )

fld_sheet <- fld_sheet %>%
  mutate(site_id = as.character(site_id), lake_id = as.character(lake_id))
...
eb_data <- full_join( # keep all data.  Will assume GC data if not present (e.g. insufficient volume for sample)
 gc_lakeid_agg, # GC data
  ...
  left_join(., # keep all data from above, discard dg_sheet data that don't match up 
            dg_sheet %>% select(lake_id, site_id,
...
eb_data <- eb_data %>%
  mutate(
    trap_rtrvl_date_time = as.POSIXct(paste(trap_rtrvl_date, trap_rtrvl_time), format = "%Y-%m-%d %H:%M:%S"),
    trap_deply_date_time = as.POSIXct(trap_deply_date_time)  # ensure it's also POSIXct
  )
...
my_eb_list <- list()
unique_cases <- eb_data %>% distinct(lake_id, site_id)

for (i in 1:nrow(unique_cases)) {
  case.i <- unique_cases[i, ]
  
  data.i <- eb_data %>%
    filter(lake_id == case.i$lake_id, site_id == case.i$site_id)
  
  out.ch4 <- mass.rate(data.i, choice1 = "ch4") 
  out.co2 <- mass.rate(data.i, choice1 = "co2")
  out.n2o <- mass.rate(data.i, choice1 = "n2o")
  
  my_eb_list[[i]] <- data.frame(
    eb_ch4_mg_m2_h = out.ch4,
    eb_co2_mg_m2_h = out.co2,
    eb_n2o_mg_m2_h = out.n2o,
    lake_id = case.i$lake_id,
    site_id = case.i$site_id
  )
}
